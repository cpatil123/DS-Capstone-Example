...
WordLineChar.df <- data.frame('Dataset' = c("Blogs", "News", "Twitter"), 'Raw Size (MB)' = c(blogs.size, news.size, twitter.size), 'Lines' = c(Blogs.Lines, News.Lines, Twitter.Lines), 'Words' = c(0, 0, 0), 'Characters' = c(CharLengthOfBlogsText, CharLengthOfNewsText,  CharLengthOfTwitterText))
colnames(WordLineChar.df) <- c("Dataset", "Size (MB)", "Line Count", "Word Count", "Character Count")
...
s.WordLineChar.df <- data.frame('Dataset' = c("Blogs", "News", "Twitter"), 'Sample Size (MB)' = c(s.blogs.size, s.news.size, s.twitter.size), 'Lines' = c(s.Blogs.Lines, s.News.Lines, s.Twitter.Lines), 'Words' = c(0, 0, 0), 'Characters' = c(s.CharLengthOfBlogsText, s.CharLengthOfNewsText,  s.CharLengthOfTwitterText))
colnames(s.WordLineChar.df) <- c("Dataset", "Sample Size (MB)", "Line Count", "Word Count", "Character Count")
```
```{r Sampled WordLineChar Table, echo=FALSE, message=FALSE, warning=FALSE}
kable(s.WordLineChar.df)
```
